...
function handleAnswer(selectedIndex) {
    ...
    const logData = {
        ...
        userSelected: (selectedIndex !== null) ? teksJawabanUser : "Waktu Habis",
        isCorrect: isCorrect, 
        indexJawabanUser: selectedIndex,
        kunciIndex: q.answer,
...
function finishQuiz() {
    ...
    summaryHTML += `
        ...
        <div id="review-pembahasan-area" class="hidden" style="margin-top: 30px; text-align: left;">
            <h3 style="color: var(--accent); border-bottom: 2px solid #f3ebff; padding-bottom: 10px;">
                <i class="fas fa-spell-check"></i> Lembar Pembahasan
            </h3>

            <div style="display: flex; gap: 10px; margin: 15px 0;">
                <button onclick="filterReview('all')" id="btn-filter-all" 
                    style="flex:1; padding: 8px; border-radius: 8px; border: 1px solid var(--accent); background: var(--accent); color: white; cursor: pointer; font-size: 0.8rem; font-weight: 600;">
                    Semua
                </button>
            ...
            <div id="review-list-container" class="review-scroll-container">
            </div>
        </div>
    `;

    // --- TAMBAHAN LOGIKA BRANDING SPONSOR ---
    if (typeof activeMotivation !== 'undefined' && activeMotivation !== "") {
        const motivasiBox = document.getElementById('motivation-display');
        if (motivasiBox) {
            motivasiBox.innerHTML = `
                <div style="margin-top:25px; padding:20px; border-radius:12px; background: var(--accent-light); border: 1px solid var(--accent); position: relative; overflow: hidden; animation: slideUp 0.5s ease;">
                    <span style="position: absolute; top: -10px; left: 10px; font-size: 4rem; color: var(--accent); opacity: 0.2; font-family: serif;">&ldquo;</span>
                    
                    <p style="font-style: italic; color: #494D5F; line-height: 1.6; margin-bottom: 10px; position: relative; z-index: 1;">
                        ${activeMotivation}
                    </p>
                    
                    <div style="display: flex; align-items: center; gap: 8px; justify-content: flex-end;">
                        <div style="height: 2px; width: 20px; background: var(--accent);"></div>
                        <small style="font-weight: 700; color: var(--accent); text-transform: uppercase; letter-spacing: 1px; font-size: 0.65rem;">
                            Pesan dari Sponsor
                        </small>
                    </div>
                </div>
...
function updatePagination() {
    ...
    const current = currentQuestionIndex; // 0-based index

    // 1. Tombol PREVIOUS (Panah Kiri)
    ...
    // 2. LOGIKA LIMIT 5 TOMBOL ANGKA
    // Menghitung angka awal dan akhir agar halaman aktif selalu di tengah
    let startPage = Math.max(0, current - 2);
    let endPage = Math.min(totalSoal - 1, startPage + 4);

    // Penyesuaian jika sudah di ujung akhir soal
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        // CSS Class: Aktif, Sudah Dijawab, atau Normal
        btn.className = (i === current) ? 'nav-btn active' : 'nav-btn';
        if (userAnswersLog[i]) {
    ...
    // 3. Tombol NEXT (Panah Kanan)
    const btnNext = document.createElement('button');
    btnNext.className = 'nav-btn';
    btnNext.innerHTML = '<i class="fas fa-chevron-right"></i>';
    btnNext.disabled = current === totalSoal - 1;
    btnNext.onclick = () => {
        currentQuestionIndex++;
        showQuestion();
    };
...
window.filterReview = function(type) {
    ...
    const container = document.getElementById('review-list-container'); // Ambil container scroll
    ...
    // MODIFIKASI 1: Reset posisi scroll ke paling atas setiap kali filter diklik
    if (container) {
        container.scrollTop = 0;
    }

    // MODIFIKASI 2: Styling tombol yang lebih kontras
    if (type === 'all') {
        btnAll.style.background = "#8458B3"; btnAll.style.color = "white";
        btnAll.style.borderColor = "#8458B3";
        
        btnWrong.style.background = "white"; btnWrong.style.color = "#666";
        btnWrong.style.borderColor = "#eee";
    } else {
        btnWrong.style.background = "#e74c3c"; btnWrong.style.color = "white";
        btnWrong.style.borderColor = "#e74c3c";
        
        btnAll.style.background = "white"; btnAll.style.color = "#666";
        btnAll.style.borderColor = "#eee";
    }
};
...
//MANAJEMEN IKLAN DI KUIS
database.ref('settings/ads/branding').once('value', (snapshot) => {
    const today = new Date().toISOString().split('T')[0];
    snapshot.forEach((child) => {
        const ad = child.val();
        if (today >= ad.startDate && today <= ad.endDate) {
            // Pasang Logo di atas container input nama
            const adArea = document.getElementById('quiz-ad-branding');
            if(adArea) {
                adArea.innerHTML = `
                    <div style="text-align:center; margin-bottom:15px;">
                        <span style="font-size:0.8rem; color:#666;">Belajar Bersama</span><br>
                        <img src="${ad.logoUrl}" style="max-height:40px; margin-top:5px;">
                    </div>
                `;
            }
            
            // Simpan Motivasi untuk akhir kuis
            window.adMotivation = ad.motivation;
            window.adClientName = ad.client;
        }
    });
});

// SAAT SELESAI KUIS (Sebelum Skor Muncul)
function showFinishBranding() {
    if (window.adMotivation) {
        alert(`Terima kasih telah mengerjakan kuis bersama ${window.adClientName}.\n\n"${window.adMotivation}"`);
    }
}
...
function resetBrandingStyles() {
    ...
    root.style.setProperty('--accent', '#8458B3');
    root.style.setProperty('--accent-light', '#DDBDF4');
